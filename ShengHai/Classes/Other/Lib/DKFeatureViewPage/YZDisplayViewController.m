...
- (void)setUpAllTitle
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        if(i != count - 1 && self.needsCarve) {
            UIView *view = [[UIView alloc] initWithFrame:CGRectMake(CGRectGetMaxX(label.frame) + 10, label.y , 1, label.height * 0.4)];
            view.centerY = label.centerY;
            view.backgroundColor = DKRGBColor(230, 230, 230);
...
//    CGFloat bottom = self.tabBarController == nil?0:49;
    ...
    if ([vc isKindOfClass:[UITableViewController class]]) {
        ...
        tableViewVc.tableView.contentInset = UIEdgeInsetsMake(top, 0, self.bottomInset?:0, 0);
